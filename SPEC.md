...
4. **eslint.** ~~Requires settling style questions the repos have answered in opposite directions.~~ **Narrowed by measuring — not reversed.** Diffing `eslint --print-config` between the repos: on `nuxt.config.ts`, **358 of 361 rules are identical and zero severities disagree**; on `app/app.vue`, 487 of 517. The headline is agreement, and reading it as *settled* would be this section's own error again — the residue is small but it is precisely the disputed part. Three findings. **(a) The style dispute is real, and it is three rules**: `vue/component-name-in-template-casing` (application repo `PascalCase`, template `kebab-case`), `vue/block-order` (application repo `template, style, script`; template `template, script, style`), and `vue/html-self-closing` (application repo `warn`; template `error` with `component: 'never'` and a written rationale — CDN snippets need explicit closing tags, since native HTML does not honour self-closing on custom elements). All three are *explicitly set in both repos*: decisions, held in opposite directions, not defaults that happened to drift. **(b) Ten of the eleven severity disagreements are the documented `vue-a11y` suppressions**, already owned by item 1 and [ADR-0009](./docs/adr/0009-the-accessibility-gap-is-three-rules.md); the eleventh is `vue/html-self-closing`, above. **(c) The 15-rule gap on `app/app.vue` is version skew, and it is a silent loss of coverage.** `@antfu/eslint-config` 9.2.0 (application repo) scopes its unicorn block to `files: [GLOB_SRC]` — `**/*.?([cm])[jt]s?(x)`, which excludes `.vue`; 9.0.0 (template) leaves the block unscoped. Fifteen unicorn rules therefore stopped applying to the application repo's 301 Vue SFCs at upgrade, with nothing in either config to say so. Re-enabling them finds **0 violations today**, so nothing is broken — but the coverage went away unchosen, which is exactly the hazard item 8 anticipates, arriving early and in a repo that already lints in CI. Still blocked, then, on something far smaller than this entry claimed: three deliberate rules.
